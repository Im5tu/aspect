...
{
    public class AwsVpcPeeringConnectionResourceExplorerTests
    {
        [Fact]
        public void ShouldHaveCorrectType()
        {
            GetTarget(out _).ResourceType.Should().Be(typeof(AwsVpcPeeringConnection));
        }
        ...
        public async Task ShouldMapAllPropertiesWhenNotNull()
        {
            ...
            resources.Should().HaveCount(1).And.Subject.First().Should().BeOfType<AwsVpcPeeringConnection>();
            var sut = (AwsVpcPeeringConnection)resources[0];
            sut.Type.Should().Be(nameof(AwsVpcPeeringConnection));
        ...
        private IResourceExplorer<AwsAccount, AwsAccount.AwsAccountIdentifier> GetTarget(out Mock<IAmazonEC2> client)
        {
            ...
            return new AwsVpcPeeringConnectionResourceExplorer(_ => ec2Client);
        }
    }
}
