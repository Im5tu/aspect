...
{
    public class AwsVpcEndpointResourceExplorerTests
    {
        [Fact]
        public void ShouldHaveCorrectType()
        {
            GetTarget(out _).ResourceType.Should().Be(typeof(AwsVpcEndpoint));
        }
        ...
        public async Task ShouldMapAllPropertiesWhenNotNull()
        {
            ...
            resources.Should().HaveCount(1).And.Subject.First().Should().BeOfType<AwsVpcEndpoint>();
            var sut = (AwsVpcEndpoint)resources[0];
            sut.Type.Should().Be(nameof(AwsVpcEndpoint));
        ...
        private IResourceExplorer<AwsAccount, AwsAccount.AwsAccountIdentifier> GetTarget(out Mock<IAmazonEC2> client)
        {
            ...
            return new AwsVpcEndpointResourceExplorer(_ => ec2Client);
        }
    }
}
